...
function AutoDriveDebugSettingsEvent:writeStream(streamId)
    -- g_logManager:info("[AD] AutoDriveDebugSettingsEvent.writeStream currentDebugChannelMask %s", tostring(self.currentDebugChannelMask))
...
function AutoDriveDebugSettingsEvent:readStream(streamId, connection)
    ...
    -- g_logManager:info("[AD] AutoDriveDebugSettingsEvent.readStream currentDebugChannelMask %s", tostring(self.currentDebugChannelMask))
    self:run(connection)
end
...
function AutoDriveDebugSettingsEvent:run(connection)
    if g_server ~= nil and connection:getIsServer() == false then
        ...
        -- g_logManager:info("[AD] AutoDriveDebugSettingsEvent.run If the event is coming from a client... currentDebugChannelMask %s", tostring(self.currentDebugChannelMask))
        AutoDriveDebugSettingsEvent.sendEvent(self.currentDebugChannelMask)

        local user = g_currentMission.userManager:getUserByConnection(connection)
        if user ~= nil and user.nickname ~= nil  then
            g_logManager:info("[AD] DebugChannel changed by user: %s", tostring(user.nickname))
        end
...
function AutoDriveDebugSettingsEvent.sendEvent(currentDebugChannelMask)
    ...
    if g_server ~= nil then
        ...
        -- g_logManager:info("[AD] AutoDriveDebugSettingsEvent.sendEvent Server have to broadcast to all clients and himself currentDebugChannelMask %s", tostring(currentDebugChannelMask))
        g_server:broadcastEvent(event, true)
    else
        -- Client have to send to server
        -- g_logManager:info("[AD] AutoDriveDebugSettingsEvent.sendEvent Client have to send to server currentDebugChannelMask %s", tostring(currentDebugChannelMask))
...
function AutoDriveDebugSettingsEvent.sendToClient(connection, currentDebugChannelMask)
    if g_server ~= nil then
        -- g_logManager:info("[AD] AutoDriveDebugSettingsEvent.sendToClient g_server ~= nil currentDebugChannelMask %s", tostring(currentDebugChannelMask))
