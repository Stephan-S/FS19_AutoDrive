...
function AutoDriveBGA:moveShovelToTarget(vehicle, target, dt)
    ...
    if shovelTargetAngleReached then
    ...
    if (vehicle.bga.armMain ~= nil and (math.abs(vehicle.bga.armMain.curRot[1] - vehicle.bga.armMain.rotMax) <= 0.01 or math.abs(vehicle.bga.armMain.curRot[1] - vehicle.bga.armMain.rotMin) <= 0.01) ) and 
    (vehicle.bga.armExtender == nil or math.abs(vehicle.bga.armExtender.curTrans[3] - vehicle.bga.armExtender.transMax) <= 0.01) then
...
function AutoDriveBGA:isAlmostInBunkerSiloArea(vehicle, distanceToCheck)
	...
	boundingBox[2] ={ 	x = x - (width/2) * ortho.x,
                        y = y+2,
						z = z - (width/2) * ortho.z};
	boundingBox[3] ={ 	x = x - (width/2) * ortho.x +  (length/2 + distanceToCheck) * vehicleVector.x,
                        y = y+2,
						z = z - (width/2) * ortho.z +  (length/2 + distanceToCheck) * vehicleVector.z };
	boundingBox[4] ={ 	x = x + (width/2) * ortho.x +  (length/2 + distanceToCheck) * vehicleVector.x,
                        y = y+2,
                        z = z + (width/2) * ortho.z +  (length/2 + distanceToCheck) * vehicleVector.z};
...
function AutoDriveBGA:driveToSiloCommonPoint(vehicle, dt)
    ...
    if self:getDistanceToTarget(vehicle) <= 3 then
...
function AutoDriveBGA:driveToSiloClosePoint(vehicle, dt)
    ...
    if self:getDistanceToTarget(vehicle) <= 2 then
