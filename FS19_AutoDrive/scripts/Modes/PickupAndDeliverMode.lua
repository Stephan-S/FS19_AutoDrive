		...
		if AutoDrive.getSetting("distributeToFolder", self.vehicle) and AutoDrive.getSetting("useFolders") then
			...
			if AutoDrive.getSetting("syncMultiTargets") then
				...
				if nextTarget ~= nil then
					...
					AutoDrive.debugPrint(self.vehicle, AutoDrive.DC_PATHINFO, "[AD] PickupAndDeliverMode:getNextTask setSecondMarker -> nextTarget getSecondMarkerName() %s", tostring(self.vehicle.ad.stateModule:getSecondMarkerName()))
				end
			elseif forced then
                -- forced is used by handleFinishedTask
				self.vehicle.ad.stateModule:setNextTargetInFolder()
				AutoDrive.debugPrint(self.vehicle, AutoDrive.DC_PATHINFO, "[AD] PickupAndDeliverMode:getNextTask setNextTargetInFolder getSecondMarkerName() %s", tostring(self.vehicle.ad.stateModule:getSecondMarkerName()))
			end
			AutoDrive.debugPrint(self.vehicle, AutoDrive.DC_PATHINFO, "[AD] PickupAndDeliverMode:getNextTask getSecondMarkerName() %s", tostring(self.vehicle.ad.stateModule:getSecondMarkerName()))

			if fillLevel > 1 then
				-- if fill material left - go to unload
				AutoDrive.debugPrint(self.vehicle, AutoDrive.DC_PATHINFO, "[AD] PickupAndDeliverMode:getNextTask set STATE_PICKUP")
				self.state = PickupAndDeliverMode.STATE_PICKUP
			else
