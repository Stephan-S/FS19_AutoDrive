    ...
    public static class MoveNodeChanger implements Changeable{
        ...
        public void undo(){
            for (int i = 0; i <= this.moveNodes.size() - 1 ; i++) {
                ...
                if (AutoDriveEditor.DEBUG) LOG.debug("Moved {}", mapNode);
        ...
        public void undo(){
            for (int j = 0; j < this.nodeListToDelete.size(); j++) {
                ...
                if (insertNode.node.id != insertNode.nodeIDbackup) {
                    if (AutoDriveEditor.DEBUG) LOG.info("## RemoveNode Undo ## ID mismatch.. correcting ID {} -> ID {}", insertNode.node.id, insertNode.nodeIDbackup);
    ...
    public static class LinearLineChanger implements Changeable{
        ...
        public LinearLineChanger(MapNode from, MapNode to, LinkedList<MapNode> inbetweenNodes, int type){
            ...
            for (int j = 0; j < inbetweenNodes.size(); j++) {
                ...
                if (AutoDriveEditor.DEBUG) LOG.info("## LinearLineChanger ## Adding ID {} to autoGeneratedNodes", toStore.id);
        ...
        public void undo(){
            ...
            if (this.autoGeneratedNodes.size() <= 2 ) {
                if (AutoDriveEditor.DEBUG) LOG.info("## LinearLineChanger.undo ## No autoGeneratedNodes - restoring to/from connections");
                ...
                for (int j = 1; j < this.autoGeneratedNodes.size() - 1 ; j++) {
                    ...
                    if (AutoDriveEditor.DEBUG) LOG.info("## LinearLineChanger.undo ## undo is removing ID {} from MapNodes", toDelete.id);
                    MapPanel.roadMap.removeMapNode(toDelete);
                    if (storedNode.hasChangedID()) {
                        if (AutoDriveEditor.DEBUG) LOG.info("## LinearLineChanger.undo ## Removed node changed ID {}", storedNode.mapNode.id);
                        storedNode.resetID();
                        if (AutoDriveEditor.DEBUG) LOG.info("## LinearLineChanger.undo ## Reset ID to {}", storedNode.mapNode.id);
                    }
        ...
        public void redo(){
            ...
            if (this.autoGeneratedNodes.size() > 2 ) {
                for (int i = 1; i < this.autoGeneratedNodes.size() - 1 ; i++) {
                    ...
                    if (AutoDriveEditor.DEBUG) LOG.info("## LinearLineChanger.redo ## Inserting ID {} in MapNodes", newNode.id);
    ...
    public static class QuadCurveChanger implements Changeable{
        ...
        public QuadCurveChanger(LinkedList<MapNode> curveNodes, boolean isReverse, boolean isDual){
            ...
            for (int i = 0; i <= curveNodes.size() -1 ; i++) {
                ...
                if (AutoDriveEditor.DEBUG) LOG.info("## QuadCurveChanger ## Adding ID {} to storedCurveNodeList", mapNode.id);
        ...
        public void redo(){
            ...
            for (int i = 1; i <= this.storedCurveNodeList.size() - 2 ; i++) {
                ...
                if (AutoDriveEditor.DEBUG) LOG.info("## QuadCurveChanger ## Inserting mapNode ID {}", curveNode.mapNode.id);
